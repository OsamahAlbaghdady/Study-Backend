...
EOF
    ...
    dtoContent=$(
        cat <<EOF
namespace BackEndStructuer.DATA.DTOs.${entityName}Dto
{
...
EOF
    ...
    updateContent=$(
        cat <<EOF
namespace BackEndStructuer.DATA.DTOs.${entityName}Update
{
...
EOF
    ...
    formContent=$(
        cat <<EOF
namespace BackEndStructuer.DATA.DTOs.${entityName}Form
{
...
EOF
    ...
    filterContent=$(
        cat <<EOF
namespace BackEndStructuer.DATA.DTOs.${entityName}Filter
{
...
using AutoMapper;
using BackEndStructuer.DATA.DTOs.${entityName}Dto;
using BackEndStructuer.DATA.DTOs.${entityName}Filter;
using BackEndStructuer.DATA.DTOs.${entityName}Form;
using BackEndStructuer.DATA.DTOs.${entityName}Update;
using BackEndStructuer.Entities;
...
namespace BackEndStructuer.Controllers
{
    public class ${entityName}sController : BaseController
    {
        ...
        ${auth}
        [HttpGet]
        public async Task<ActionResult<List<${entityName}Dto>>> GetAll([FromQuery] ${entityName}Filter filter) => Ok(await _$(echo "$entityName" | tr '[:upper:]' '[:lower:]')Services.GetAll(filter));
...
EOF
    ...
    createJobFailedException "DataContext.cs" "${dbContextPath}" "$dbContextContent"
    ...
    createJobFailedException "$formName.cs" "$dataForm/${entityName}Form.cs" "$updateContent"
    createJobFailedException "$updateName.cs" "$dataForm/${entityName}Update.cs" "$formContent"
...
CreateMap<${entityName%%.cs}Update,${entityName%%.cs}>();
EOF
    )
